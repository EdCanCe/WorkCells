	...
	OneToOne.countVariables()
		.then(([rows]) => {
			...
			for(let i = 1; i <= questionsNum; i++){
				const answer = new Answer(req.body[`question_${i}`], req.params.sessionID, req.body[`question_id_${i}`]);
				console.log("RSPUESTA: ");
				console.log(req.body[`question_${i}`]);
				answer.save();
			}
			for(let i = 1; i <= measurableAmount; i++){
				const measure = new Measure(req.body[`measure_${i}`], req.params.sessionID, req.body[`measurable_id_${i}`])
				measure.save();
			}
		...
		.catch((err) => {
			console.log(err);
			req.session.info = err.message || "There was an error trying to sumbit it.";
